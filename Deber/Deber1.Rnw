...
\begin{enumerate}
% Ejercicio 1
\item[\bf{1.}] Demestre que la familia de distribuciones $Gamma(\alpha,\lambda)$ con $\alpha,\lambda$
desconocidos es una familia exponencial.

% Ejercicio 2
\item[\bf{2.}] Demestre que la familia de distribuciones $Binomial(n,\theta)$ con $\theta$ desconocido
es una familia exponencial. Encuentre la esperanza y la varianza del estadístico $T(X)$ respectivo.
...
% Ejercicio 4
\item[\bf{2.}] Suponga que $X_1,\dots,X_n$ son variables aleatorias $i.i.d.$ uniformes en el intervalo $[0,\theta]$
...
% Ejercicio 6
\item[\bf{4.}] Suponga que $X_1,\dots,X_n$ son variables aleatorias $i.i.d.$ con función de densidad:
...
\begin{enumerate}
\item[\bf{a.}] Demuestre que $Z_{(n)} = min(X_1,\dots,X_n)$ es un estadístico suficiente para $\theta$. 
\item[\bf{b.}] Demuestre que $Z_{(n)}$ converge en probabilidad a $\mu$ cuando $n \rightarrow \infty$
\end{enumerate}
