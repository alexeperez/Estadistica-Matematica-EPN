...
donde $h(x)$ es una función continua, conocida, definida en los reales.
...
donde $Z_1$ y $Z_2$ son variables independientes exponenciales de parámetros $\lamda_1 a(\theta_1,
\theta_2)$ y $\lamda_2 a(\theta_1,\theta_2)$ respectivamente.
\end{enumerate}


% Ejercicio 2
\item[\bf{2.}] Sean $X_1,\dots,X_n$ variables aleatorias $i.i.d.$ con función de densidad:
\begin{equation*}
f(x_i; \theta) = \frac{x}{\theta^2} e^{-x/\theta}
\end{equation*}
\begin{enumerate}
...
\begin{equation*}
        f(x; \theta) = \frac{1}{\sigma} exp[-\frac{x-\mu}{\sigma}] \hspace{1cm}para\, x \geq \mu
\end{equation*}
...
\item[\bf{b.}] Si $\mu=2$ y $\sigma$ halle un MLE para $\sigma$.
\end{enumerate}
...
\item[\bf{b.}] Si $\beta$ es conocido y $\alpha$, encuentre un MLE para $\alpha$.
\end{enumerate}

% Ejercicio 6
\item[\bf{6.}] Considere el modelo de regresión lineal múltiple definido como:
\begin{equation*}
        Y_i = \beta_0 + \beta_1 x_{i2}+ \beta_1 x_{i3}+ \dots +  \beta_k x_{ik} + u_i \hspace{1cm} para\, i=1,\dots, n.
\end{equation*}
Suponiendo que $u_i -> N(0, \sigma^2)$, encuentre MLEs para $\beta_1, \dots, \beta_k$ basados en
$Y_1,\dots, Y_n$.
